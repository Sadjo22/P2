...
Computer_Engineering::Computer_Engineering(const string& sname, const string& snum, const string& fn,const string& loc, int con, const string& deg, int ms, int is):
    Engineering(sname,snum,fn,loc,con),Degree(deg),mathScore(ms),logicScore(is){}
...
string Computer_Engineering::respo="MIKE KARINE";


string Computer_Engineering::getResponsibleName()const{
    return respo;
}


bool Computer_Engineering::access(int mat,int log)const{

    return (mat >=mathematics  &&  log >= logics);
}
